...
21. **Phase 21:** Variable Prompts — user-managed **Prompt Sets** replace the global `RECOMMENDATION_INSTRUCTION` / `SESSION_MEMORY_INSTRUCTION` settings; each set has a name, a single recommendation prompt, and a `useSession` toggle; full CRUD in the Settings modal under "Prompt Sets"; header dropdown (after location, before session bar) activates a set — `useSession=false` clears chat/recs and greys out the session toolbar showing 0/0, `useSession=true` restores that set's isolated session; `session_events` tagged with `prompt_set_name`; active reply ID namespaced per set (`active_reply_id:<name>` in `session_state`); active set persisted as `ACTIVE_PROMPT_SET_NAME` in `app_settings`; on first upgrade the "Default" set is seeded with the full inline recommendation instruction (the `{sessionMemoryInstruction}` token is resolved to its literal text via migration `20260603000002`); only `{minTracks}` and `{maxTracks}` tokens are substituted at call time
22. **Phase 22:** Automatic Response Format Appendix — JSON output format rules (response shape, bold-asterisk formatting of track names, min/max track count, "return valid JSON only") extracted from the user-editable prompt set text; stored in `AiPromptDefaults.ResponseFormatAppendix` (never persisted to DB, never user-visible); `SubstituteTrackTokens` renamed to `BuildSystemInstruction` in `IAiPromptService` / `AiPromptService` — the method now substitutes `{minTracks}`/`{maxTracks}` in the user's template then appends the format appendix; applied universally to every prompt set regardless of content; migration `20260603000003` strips the old JSON-rules block from existing `prompt_sets` rows via exact-match `REPLACE()`
