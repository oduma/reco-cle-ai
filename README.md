...
15. **Phase 15:** Safe DB upgrades — EF Core migrations introduced (hybrid mode: repositories keep raw ADO.NET, EF tracks migration history only); `DatabaseBaseline` stamps existing databases; `CleanupRenamedPromptKeys` migration renames legacy setting keys
16. **Phase 16:** Environmental context — voice toggle moved to Settings modal; header shows current city/country via IP geolocation; "Use User Location" and "Use Current Weather" checkboxes in Settings; when enabled, location and WMO weather code appended to each recommendation prompt
17. **Phase 17:** Recommendation history no-repeat — persistent `recommendation_history` table; every AI-suggested batch written to the table; last 100 tracks injected into every prompt as a "do not suggest again" block; table capped at a configurable row limit
18. **Phase 18:** Settings improvements — per-field reset-to-default buttons and "Leave blank to use default" hints for all non-secret settings; defaults served from the API (not hardcoded in Angular); `floatLabel="always"` on every form field eliminates the label-clipping rendering bug
19. **Phase 19:** Settings DB seeding — all non-secret defaults seeded into `app_settings` on startup; new `app_setting_defaults` table stores canonical defaults; `AppSettingsService` bypasses env vars for non-secret settings; only `GEMINI_API_KEY`, `LASTFM_API_KEY`, and optionally `REASONIC_DB_PATH` are needed in `.env.local`
20. **Phase 20:** Avoid-artists filter — new "Personalization" section in the settings modal with a multi-line textarea for artists the user never wants recommended (one per line); list stored in `app_settings`; injected into the recommendation prompt as a permanent avoid block; returned tracks also filtered server-side to strip any match
