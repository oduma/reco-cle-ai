...
1. Uses **Gemini** (cloud) or **Ollama** (local) as the LLM. The LLM is both the recommender and the explanation generator — it produces the conversational narrative and the structured track list in one call.

2. Does **not** use external metadata providers (MusicBrainz, Last.fm, Discogs). The explanation is grounded entirely in what the LLM already knows about music.

3. Annotates tracks against the **Clementine-backed local inventory** using fuzzy matching. The `inLocalLibrary` flag is the primary signal distinguishing owned tracks from discovery suggestions.

Because explanation is generated by the LLM itself, the system should:
- prompt the LLM to explain its recommendations concisely and musically,
- let the narrative explain why each track was chosen,
- and surface the `inLocalLibrary` flag clearly in the UI so the user always knows what they own.
