...
public class DiaryService : IDiaryService
{
    ...
    public async Task<DiaryEntryResponse> GetOrGenerateEntryAsync(
        string date, bool force, CancellationToken cancellationToken = default)
    {
        ...
        _logger.LogInformation("[Diary] Generating entry for {Date} ({PromptCount} prompts)", date, dayData.Prompts.Count);

        var content = await _gemini.GenerateDiaryEntryAsync(prompt, cancellationToken);
